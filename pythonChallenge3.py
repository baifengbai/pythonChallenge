test = """kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
import re
r = re.compile('[a-z][A-Z][A-Z][A-Z][(a-z)][A-Z][A-Z][A-Z][a-z]')

d = re.findall(r, test)

for elem in d:
	print elem[int(len(elem)/2)],
...
"""for i in range(len(test)):
	...
	if oneAhead >= len(test):
		continue
	if oneBehind < 0:
		continue

	if test[oneAhead].isupper() and test[twoAhead].isupper() and test[threeAhead].isupper() and test[fourAhead].islower() and test.[oneBehind].isupper() and test[twoBehind].isupper() and test[threeBehind].isupper() and test[fourBehind].isupper():
		print test[i], """